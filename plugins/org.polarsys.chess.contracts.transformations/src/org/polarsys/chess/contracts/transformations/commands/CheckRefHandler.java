...
public class CheckRefHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		CHESSContractValidationAndRefinementHandler handler = new CHESSContractValidationAndRefinementHandler(editor, activeShell);
		Command command = handler.getCommand();
		if(command != null){
			command.execute();
		}else{
			MessageDialog.openError(activeShell, "Contract Refinement Analysis", "Problems while perfoming analysis");
		}
		return null;
	}
