...
/**
 * The Class CreateCDOCHESSModelWizard is the implementation of the wizard used to create a new CHESS model
 */
public class CreateCDOCHESSModelWizard extends CreateModelWizard {
	...
	/** The provider pages. */
	private Map<CHESSCDONewModelStorageProvider, List<IWizardPage>> providerPages = new java.util.HashMap<CHESSCDONewModelStorageProvider, List<IWizardPage>>();
	
	/** The providers by page. */
	private Map<IWizardPage, CHESSCDONewModelStorageProvider> providersByPage = new java.util.HashMap<IWizardPage, CHESSCDONewModelStorageProvider>();
	...
	public void addPages() {
		startProviderPageIndex = getPageCount() - 1;
			...
			for (IWizardPage page : cdoStorageProvider.createPages()) {
				if (page != null)  {
					pageList.add(page);
					providersByPage.put(page, cdoStorageProvider);
					addPage(page);
				}
			}
			providerPages.put(cdoStorageProvider, pageList);
	...
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		cdoStorageProvider.init(this, selection);
	}
	...
	public boolean performFinish() {
		...
		//early version
		addProjectFileWithNature(cdoStorageProvider.getFolderRelativePath(), cdoStorageProvider.getProjectName());

		return true;
	}

	private void addProjectFileWithNature(String folderRelativePath, String modelName) {
...
//				"localhost"
		...
		CDOTransaction transaction = CDOConnectionUtil.instance.openTransaction(session);
		
		//.project file with CHESs nature
		...
		sb.append("<natures>");
		sb.append("<nature>org.polarsys.chess.CHESSNature</nature>");
		sb.append("</natures>");
		sb.append("</projectDescription>");

//		System.out.println(sb.toString());
			
		//the .project file can be implemented as CDOTextResource as it seems reasonable...
//		CDOTextResource dotProjectFile = transaction.createTextResource(folderRelativePath + "/.project");
//		try {
//			CDOClob value = new CDOClob(new StringReader(sb.toString()));
//			dotProjectFile.setContents(value);
//		} catch (IOException e1) {
//			// TODO Auto-generated catch block
//			e1.printStackTrace();
//		}
		//..however, to be consistent with the imported/exported projects, we could use a CDOBinaryResource instead:
		CDOBinaryResource dotProjectFile = transaction.createBinaryResource(folderRelativePath + "/.project");
		try {
			ByteArrayInputStream stream = new ByteArrayInputStream(sb.toString().getBytes());
			CDOBlob value = new CDOBlob(stream);
			dotProjectFile.setContents(value);
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
	...
	protected URI createNewModelURI(String diagramCategoryID) {
		return cdoStorageProvider.createNewModelURI(diagramCategoryID);
	}
	...
	protected IEditorInput createEditorInput(URI uri) {
		return cdoStorageProvider.createEditorInput(uri);
	}
	...
	public IStatus diagramCategoryChanged(String... newCategories) {
		return cdoStorageProvider.validateDiagramCategories(
			newCategories);
	}
	...
	protected void initStorageProvider(IWorkbench workbench,
		...
		cdoStorageProvider = new CHESSCDONewModelStorageProvider();
	}
	...
	public IWizardPage getNextPage(IWizardPage page) {
		...
		CHESSCDONewModelStorageProvider provider = providersByPage.get(page);
		if (provider != null) {
			...
			List<IWizardPage> pages = providerPages.get(cdoStorageProvider);
			if (!pages.isEmpty()) {
				result = pages.get(0);
			} else {
				// get the first page after the provider pages
	...
	public boolean canFinish() {
		...
		if (result) {
			for (IWizardPage next : providerPages.get(cdoStorageProvider)) {
